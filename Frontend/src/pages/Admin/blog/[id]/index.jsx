...
const categorias = ["Carreira", "Tecnologia", "Portfólio", "Produtividade", "Currículo", "Entrevista"]

export default function AdminBlogPostForm({ id }) {
  ...
  useEffect(() => {
    if (isEditing) {
      setIsLoading(true)
      // Simulação de chamada à API para buscar dados do post
      setTimeout(() => {
        setFormData({
          titulo: "Como se preparar para entrevistas técnicas de estágio",
          slug: "como-se-preparar-para-entrevistas-tecnicas-de-estagio",
          excerpt:
            "Dicas práticas para se destacar em entrevistas técnicas e conquistar a vaga de estágio dos seus sonhos.",
          conteudo: `<p>As entrevistas técnicas são uma parte crucial do processo seletivo para estágios em desenvolvimento de software. Elas avaliam não apenas seus conhecimentos técnicos, mas também sua capacidade de resolver problemas, comunicar-se efetivamente e trabalhar sob pressão.</p>
          ...
          status: "Publicado",
          dataPublicacao: "2023-05-15T10:00",
        })
        setPreviewImage("https://placehold.co/600x400/EEE/31343C")
        setIsLoading(false)
      }, 800)
    }
  }, [isEditing])
  ...
  const validateForm = () => {
    ...
    if (formData.status === "Agendado" && !formData.dataPublicacao) {
      newErrors.dataPublicacao = "Data de publicação é obrigatória para posts agendados"
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(isEditing ? "Artigo atualizado com sucesso!" : "Artigo criado com sucesso!")
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-center">
            <Link to="/admin/blog" className="text-primary hover:text-primary-dark mr-2">
            ...
            <h1 className="text-2xl font-bold text-gray-dark">{isEditing ? "Editar Artigo" : "Criar Novo Artigo"}</h1>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-soft overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Título */}
                <div className="col-span-1 md:col-span-2">
                  <label htmlFor="titulo" className="block text-sm font-medium text-gray-dark mb-1">
                ...
                {/* Slug */}
                <div className="col-span-1 md:col-span-2">
                  <label htmlFor="slug" className="block text-sm font-medium text-gray-dark mb-1">
                    Slug <span className="text-xs text-gray-500">(URL amigável)</span>
                  </label>
                  <input
                    type="text"
                    id="slug"
                ...
                {/* Resumo */}
                <div className="col-span-1 md:col-span-2">
                  <label htmlFor="excerpt" className="block text-sm font-medium text-gray-dark mb-1">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    className={`block w-full px-3 py-2 border ${
                      errors.excerpt ? "border-red-500" : "border-gray-medium"
                    } rounded-lg focus:ring-2 focus:ring-primary focus:border-primary`}
                ...
                {/* Categoria */}
                <div>
                  <label htmlFor="categoria" className="block text-sm font-medium text-gray-dark mb-1">
                ...
                {/* Tags */}
                <div>
                  <label htmlFor="tags" className="block text-sm font-medium text-gray-dark mb-1">
                ...
                {/* Autor */}
                <div>
                  <label htmlFor="autor" className="block text-sm font-medium text-gray-dark mb-1">
                ...
                {/* Cargo do Autor */}
                <div>
                  <label htmlFor="autorRole" className="block text-sm font-medium text-gray-dark mb-1">
                    Cargo do Autor
                  </label>
                  <input
                    type="text"
                    id="autorRole"
                    name="autorRole"
                    value={formData.autorRole}
                    onChange={handleChange}
                    className="block w-full px-3 py-2 border border-gray-medium rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                    placeholder="Ex: Recrutadora Senior"
                  />
                </div>
                ...
                {formData.status === "Agendado" && (
                  <div>
                    <label htmlFor="dataPublicacao" className="block text-sm font-medium text-gray-dark mb-1">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`block w-full px-3 py-2 border ${
                        errors.dataPublicacao ? "border-red-500" : "border-gray-medium"
                      } rounded-lg focus:ring-2 focus:ring-primary focus:border-primary`}
                    />
                    {errors.dataPublicacao && <p className="mt-1 text-sm text-red-500">{errors.dataPublicacao}</p>}
                  </div>
                )}

                {/* Imagem de Capa */}
                <div className="col-span-1 md:col-span-2">
                  <label htmlFor="imagem" className="block text-sm font-medium text-gray-dark mb-1">
                    Imagem de Capa
                  </label>
                  ...
                  {previewImage && (
                    <div className="mt-3">
                      ...
                      <div className="relative h-40 w-full md:w-1/2 rounded-lg overflow-hidden">
                        <img src={previewImage || "/placeholder.svg"} alt="Preview" fill className="object-cover" />
                      </div>
                    </div>
                  )}
                </div>
                ...
                {/* Conteúdo */}
                <div className="col-span-1 md:col-span-2">
                  <label htmlFor="conteudo" className="block text-sm font-medium text-gray-dark mb-1">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Dica: Você pode usar tags HTML básicas como &lt;h3&gt;, &lt;p&gt;, &lt;strong&gt;, &lt;em&gt;,
                    &lt;ul&gt;, &lt;li&gt;, etc.
                  </p>
                </div>
              </div>
